...
class Game {
  ...
  async load() {
    ...
    this.textures = await this.loadTexturesFromAssets();
    return this.onLoad();
  }

  async loadTexturesFromAssets() {
    const imageContext = require.context('../assets/images', true, /\.png$/);
    const keys = imageContext.keys();
    const loadedEntries = await Promise.all(keys.map(async (contextPath) => {
      const texturePath = contextPath.replace('./', '');
      const assetUrl = imageContext(contextPath);
  ...
  onLoad() {
    // Pixel art atlas: nearest sampling avoids bleeding between packed frames.
